...
class Application:
    def __init__(self):
        print("[INFO]: starting...")
        # turn on webcam when class is called
        self.vs = None # capture video frames
    ...
    def creat_widgets(self):
        # WindowVideoCapture
        self.root.title("Demo") # set titleVideoCapture
    ...
    def video_loop(self):
        self.vs = cv2.VideoCapture('http://10.131.11.110:8080/shot.jpg')
        ...
        if ok:  # frame captured without any errors
            ...
            _, thre = cv2.threshold(small, 160, 255, cv2.THRESH_BINARY_INV) # make binary from small image
            ...
            for i in contours:
                ...
                if (w > 15 and h > 15) and (w < 100 and h < 100):
                    ...
                    _, results, _, _ = self.model.findNearest(np.float32(roi), 10) # predict
                    pre = chr(results)
